...
public class viewrecSlice extends AbilitySlice implements Component.ClickedListener {
    RdbStore store;
    ...
    Vector<PixelMap> getsingelpic(singeldetail singeldetail){
        ...
        byte[] picbyte = singeldetail.getPicbyte();
    ...
    void viewbydate(String seldate){
        ...
        RdbPredicates rdbPredicates = new RdbPredicates("test").equalTo("date",seldate).equalTo("type",1).orderByDesc("date");
        ResultSet resultSet = store.query(rdbPredicates,columns);
        if(resultSet.getRowCount()!=0){
            resultSet.goToFirstRow();
            for(int i=0;i<resultSet.getRowCount();i++){
                singeldetail singeldetail = new singeldetail();
...
//            showpic.setMarginLeft(260);
    ...
    void viewbycity(String city){
        ...
        Vector<singeldetail>vector = new Vector<singeldetail>();
        String[] columns = new String[] {"id","type","picdes","sitdes","weather","date","title","des","picbyte","city","like"};
        RdbPredicates rdbPredicates = new RdbPredicates("test").beginsWith("city",city).equalTo("type",1).orderByDesc("date");
        ResultSet resultSet = store.query(rdbPredicates,columns);
        if(resultSet.getRowCount()!=0){
            resultSet.goToFirstRow();
            for(int i=0;i<resultSet.getRowCount();i++){
...
//            showpic.setMarginLeft(260);
    ...
    void viewbylike(int islike){
        ...
        Vector<singeldetail>vector = new Vector<singeldetail>();
        String[] columns = new String[] {"id","type","picdes","sitdes","weather","date","title","des","picbyte","city","like"};
        RdbPredicates rdbPredicates = new RdbPredicates("test").equalTo("like",islike).equalTo("type",1).orderByDesc("date");
        ResultSet resultSet = store.query(rdbPredicates,columns);
        if(resultSet.getRowCount()!=0){
            resultSet.goToFirstRow();
            for(int i=0;i<resultSet.getRowCount();i++){
...
//            seldate.setHeight(ComponentContainer.LayoutConfig.MATCH_CONTENT);
            ...
            Operation operation = new Intent.OperationBuilder()
                    .withDeviceId("")
                    .withBundleName("com.example.myapplication")
                    .withAbilityName("com.example.myapplication.slice.slice.slice.information")
                    .build();
